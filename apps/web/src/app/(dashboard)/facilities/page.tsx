...
const columns: ColumnDef<Facility>[] = [
  ...
  },
  {
    ...
    header: '',
    cell: ({ row }) => (
      <a href={`/facilities/${row.original.id}`} className="text-xs text-primary hover:underline">
        View
      </a>
    ),
  },
...
export default function FacilitiesPage() {
  ...
  const { register: regB, handleSubmit: handleB, reset: resetB, setValue: setBValue, formState: { errors: bErrors, isSubmitting: bSubmitting } } = useForm<BookingFormValues>({
    resolver: zodResolver(bookingSchema),
  });

  useEffect(() => {
    fetch('/api/v1/spatial/facilities').then((r) => r.json()).then(setFacilities).catch(() => {});
    fetch('/api/v1/spatial/bookings').then((r) => r.json()).then(setBookings).catch(() => {});
  }, []);
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={bookingOpen} onOpenChange={setBookingOpen}>
            ...
            <DialogContent>
              ...
              {bookingError && <div className="text-sm text-destructive mb-2">{bookingError}</div>}
              <form onSubmit={handleB(onSubmitBooking)} className="space-y-3">
                <div>
                  <Label>Room ID</Label>
                  <Input {...regB('roomId')} placeholder="uuid of room" />
                  {bErrors.roomId && <p className="text-xs text-destructive">{bErrors.roomId.message}</p>}
                </div>
                <div>
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <Button type="button" variant="outline" onClick={() => { setBookingOpen(false); setBookingError(null); }}>Cancel</Button>
                  <Button type="submit" disabled={bSubmitting}>Submit Booking</Button>
                </div>
              </form>
            </DialogContent>
          </Dialog>
