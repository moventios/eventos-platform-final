      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex gap-2">
            ...
            {isPending && (
              <>
                <Button size="sm" onClick={() => resolve(row.original.id, 'approved')}>
                  Approve
                </Button>
                <Button size="sm" variant="destructive" onClick={() => resolve(row.original.id, 'rejected')}>
                  Reject
                </Button>
              </>
            )}
    ...
    <div className="p-6">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage approvals for intercepted AI Agent actions</p>
        </div>
        <div className="flex gap-2">
          {['pending', 'approved', 'rejected', 'expired', 'all'].map((filter) => (
            <Button
              key={filter}
              size="sm"
          ...
          <DialogContent className="max-w-xl">
            ...
            <div className="space-y-4 text-sm mt-2">
              <div className="grid grid-cols-2 gap-4 border-b pb-4">
                <div>
                  <span className="font-semibold block text-muted-foreground">Command Type</span>
                  <span>{selectedApproval.requestContext.command_type}</span>
                </div>
                <div>
                  <span className="font-semibold block text-muted-foreground">Actor Type</span>
                  <StatusBadge status={selectedApproval.actorType} />
                </div>
                <div>
                  <span className="font-semibold block text-muted-foreground">Aggregate Type</span>
                ...
                <div>
                  <span className="font-semibold block text-muted-foreground">Expires At</span>
                  <span>{new Date(selectedApproval.expiresAt).toLocaleString()}</span>
                </div>
                <div>
                  <span className="font-semibold block text-muted-foreground">Created At</span>
              ...
              <div>
                <span className="font-semibold block text-muted-foreground mb-1">Request Payload / Context</span>
              ...
              {selectedApproval.status !== 'pending' && (
                <div className="p-3 bg-muted/50 border rounded space-y-2">
                  <h4 className="font-semibold text-muted-foreground">Resolution Info</h4>
                  {selectedApproval.resolvedAt && (
                    <p><strong>Resolved At:</strong> {new Date(selectedApproval.resolvedAt).toLocaleString()}</p>
                  )}
                  {selectedApproval.resolutionNote && (
                    <p><strong>Resolution Note:</strong> {selectedApproval.resolutionNote}</p>
                  )}
                </div>
              )}
                ...
                <div className="space-y-4 pt-2 border-t">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <Button variant="destructive" onClick={() => {
                      resolve(selectedApproval.id, 'rejected', rejectionNote || 'Rejected by Admin');
                    }}>
                      Reject Request
                    </Button>
                    <Button onClick={() => {
