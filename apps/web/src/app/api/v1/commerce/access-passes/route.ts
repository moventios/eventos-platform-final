...
import { IssueAccessPassSchema } from '@eventos/contracts';
import { IssueAccessPassHandler } from '@eventos/core/commerce';
import { DrizzleAccessPassRepository, DrizzlePassTierRepository } from '@eventos/infrastructure/postgres/commerce';
import { OutboxEventBus, createDbWithTenant } from '@eventos/infrastructure/postgres';
import { CapacityExceededError } from '@eventos/core';
import { sql, eq, and } from 'drizzle-orm';
import { accessPasses } from '@eventos/database/schema';

export const POST = withTenantContext(async (req: NextRequest, { tenantId, actorId, actorType }) => {
  ...
  if (actorType === 'AI_AGENT') {
    ...
    `);
    return NextResponse.json({ approvalId: (result as unknown as { p_approval_id: string }[])[0]?.p_approval_id }, { status: 202 });
  }
...
export const GET = withTenantContext(async (req: NextRequest, { tenantId }) => {
  ...
  if (eventId) {
    query = db.select().from(accessPasses).where(and(eq(accessPasses.tenantId, tenantId), eq(accessPasses.eventId, eventId)));
  }
