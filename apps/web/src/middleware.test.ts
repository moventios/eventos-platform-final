...
describe('middleware()', () => {
  ...
  it('resolves tenant from signed eventos_tenant_id cookie when JWT lacks claim', async () => {
    const token = makeJwt({ sub: 'actor-uuid-1' });
    getSessionImpl.mockResolvedValue({
      data: { session: { access_token: token, user: { id: 'actor-uuid-1' } } },
    });
    ...
    const req = new NextRequest('http://localhost/', {
      headers: { cookie: `eventos_tenant_id=${signed}` },
    });
    const res = await middleware(req);
  ...
  it('rejects unsigned tenant cookie values', async () => {
    ...
    const req = new NextRequest('http://localhost/', {
      headers: { cookie: 'eventos_tenant_id=foreign-tenant-id' },
    });
    const res = await middleware(req);
