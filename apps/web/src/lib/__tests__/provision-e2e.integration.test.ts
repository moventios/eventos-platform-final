...
import { memberships, profiles, tenants } from '@eventos/database/schema';
import { bindTestServiceDb, unbindTestServiceDb } from '@eventos/infrastructure/postgres';
import { createIamTestDb } from '../../../../../packages/infrastructure/src/postgres/test/create-test-db.js';
...
describe('provision e2e (real route + shipped createServiceDb)', () => {
  ...
  it('POST /api/v1/iam/tenants returns 201 and persists profile+membership', async () => {
    ...
    expect(setCookie).toContain('eventos_tenant_id');
