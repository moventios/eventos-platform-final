...
const PORT = Number(process.env['MOVENT_E2E_PORT'] ?? 3099);
...
async function main() {
  ...
  try {
    ...
    httpLog.push(`Set-Cookie present: ${setCookie.includes('movent_tenant_id')}`);
    ...
    httpLog.push(`Network contains Overview: ${dashboardHtml.includes('Overview')}`);
    httpLog.push(`Network events stat: ${eventsStat ?? 'missing'}`);
    httpLog.push(`Network facilities stat: ${facilitiesStat ?? 'missing'}`);
    ...
    if (!setCookie.includes('movent_tenant_id')) {
      throw new Error('Provision response missing movent_tenant_id Set-Cookie');
    }
    ...
    if (!dashboardHtml.includes('Overview')) {
      throw new Error('Network HTML missing Overview after provision');
    }
    if (eventsStat !== '1' || facilitiesStat !== '1') {
      throw new Error(
        `Network stats not populated (events=${eventsStat}, facilities=${facilitiesStat})`,
      );
    }
