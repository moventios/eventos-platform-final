...
- Home / Command Center + personalized "My Today + Alerts" (all personas).
  ...
  - Workflow position: Entry to all other flows.
  - Value: Immediate reduction in context switching + faster reaction.
  - UX: High-signal cards, clear next actions.
  - Eng implications: Read from existing aggregates + approvals + calendar; use realtime; fit L-07 (reads ok).
  - AI: Daily briefing.
- Approvals enhancements (already advanced) → full context, impact preview, mobile, resolution notes.
- Timeline + Resource Allocation with immediate conflict detection + suggestions (builds on current bookings + calendar).
- First AI copilot experiences: proposal / checklist / timeline draft (read-only propose + explain + "apply as starting point").
- Educational empty states, microcopy, and onboarding flows across key screens.
...
- Client / Vendor portals (lightweight views + approvals).
- Knowledge Base + grounded RAG for AI answers + template library.
- Industry starter packs (wedding, exhibition, corporate) with pre-built timelines/checklists/budgets.
...
- [x] Personas (13 cards)
- [x] JTBD (expanded)
- [x] Customer Journey (full lifecycle stages)
- [x] Information Architecture (work-centered nav + within-project)
- [x] Screen Inventory + Blueprints (purpose, audience, actions, hierarchy, states, AI)
- [x] Component Inventory (aligned to Stoic UX)
- [x] AI Copilot Blueprint (posture + specific capabilities + guardrails)
- [x] Content & Copy Blueprint (inventory + voice)
- [x] Marketing Funnel Blueprint
- [x] Implementation Roadmap + Priority Matrix (this file) with who/problem/workflow/value/eng/priority for each
