...
_Catatan: Nilai-nilai default di `.env` sudah dikonfigurasi untuk langsung kompatibel dengan Supabase CLI lokal._
...
3.  Setelah proses selesai, ketik perintah berikut untuk memverifikasi status dan melihat dashboard database (Studio):
    ...
    npx supabase status
    ```
    _Anda dapat mengakses Supabase Studio di `http://127.0.0.1:54323` untuk memantau data secara visual._
...
- **Solusi**: Buka aplikasi Docker Desktop secara manual di sistem operasi macOS Anda, tunggu hingga statusnya berubah menjadi _Running_, lalu jalankan ulang `npx supabase start`.
