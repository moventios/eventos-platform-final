...
| `Facility`         | "Venue utama didaftarkan sebagai Facility baru." (Translated to **Venue** in Frontend & routing layers). | Location, Place, Site                       | Database and Domain models use `Facility`. Frontend uses `Venue` to match product terminology.         |
| `Room`             | Domain ontology: "Facility memiliki tiga Room." (Translated to **Space** in Frontend layers).          | -                                           | Database and Domain models use `Room`. Frontend uses `Space` to match product terminology.            |
