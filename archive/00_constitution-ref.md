...
**Core Domains** _(Competitive Advantage — sumber diferensiasi utama)_
...
**Supporting Domains** _(Business Enablers — penting tapi tidak unik)_
...
**Generic Domains** _(Commoditized Utilities — gunakan solusi pihak ketiga bila mungkin)_
...
│ "Provides user_id as reference; semua domain │
...
| :------- | :------------------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **L-01** | No Cross-Context Join           | Kueri database **dilarang** melakukan `JOIN` lintas Bounded Context di level SQL. Penggabungan data dilakukan di lapisan aplikasi (CQRS Read Model atau BFF). _Pelanggaran: Review wajib sebelum merge._         |
...
| **L-05** | No Entity Without Owner         | Tidak boleh ada _orphan entity_. Setiap record wajib memiliki `tenant_id` (dan relevan: `organization_id`, `workspace_id`) yang terhubung langsung ke Aggregate Root.                                            |
...
| **L-08** | Zero-Downtime Migration         | Perubahan skema database **wajib** menggunakan pola _Expand and Contract_: tambah kolom baru → migrasi data → hapus kolom lama. Tidak boleh ada rename atau drop kolom langsung di satu migration.               |
...
### 10.4 Ledger Aggregate _(Kritikal — Transaksi Terkunci)_
...
_Triggers: `PaymentInitiated`, `PaymentCaptured`, `PaymentFailed`, `PaymentSettled`, `RefundInitiated`_
...
_Triggers: `AccessPassIssued`, `AccessPassScanned`, `AccessPassRevoked`, `AccessPassExpired`_
...
_\*Void setelah Settled memerlukan Reversal JournalEntry_
...
_JournalEntry yang `Posted` tidak pernah dihapus. Pembatalan menghasilkan entry baru dengan debit/kredit terbalik._
...
\[pgvector Store\] (PostgreSQL — terisolasi per tenant_id)
...
_Konfigurasi routing disimpan di **Prompt Registry** dan dapat diubah per Tenant tanpa deployment ulang._
...
| `AI/ML`         | Embeddings, Model outputs         | Terisolasi per tenant_id di pgvector | Akses via AI Agent dengan scope terdefinisi  |
...
Setiap ADR adalah keputusan desain yang signifikan dan dokumentasinya bersifat immutable (append-only: hanya bisa di-_supersede_ dengan ADR baru).
...
_© Sovereign OS — Dokumen ini adalah intellectual property eksklusif. Dilarang didistribusikan ke pihak eksternal tanpa izin tertulis dari Platform Owner._

_Untuk mengusulkan perubahan pada dokumen ini, ajukan RFC (Request for Constitution Change) melalui repositori internal dan dapatkan persetujuan dari Lead Architect._
